...
verified advisor logic ported from the original static instrument, which is kept in `v1/` as the working archive.
...
| Path | What it is |
| --- | --- |
| root | The Next.js website (this is what Vercel deploys). |
| `v1/` | The original single-page instrument, plain HTML, CSS and JavaScript. Run it with `python3 v1/serve.py` and open the printed address. It keeps the language-reading benchmark tooling and the recorded benchmark CSVs. |
...
The website lives at the repository root, so Vercel needs no root-directory override.

**3. Import into Vercel (free Hobby tier).**

- vercel.com, Add New Project, import the `AdviceIT` repository. Framework is detected as Next.js, defaults are
...
- The v1 static instrument in `v1/` remains the reference implementation, including the language-reading
